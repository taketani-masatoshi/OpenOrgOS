...
e-Tax 送信はユーザ承認後のみ（[tax-filing-spec.md](./tax-filing-spec.md) · ADR 0052 の 5c）。提出用データの出力は 5b。承認なしの自動送信はしない。
...
| **R3（実装済）** | 入金を GL に残せる · カレンダーに入金予定 | 公式提出 XML |
| **R4** | ユーザ承認後の e-Tax 送信（0052 5c） | 承認なしの自動送信 |
