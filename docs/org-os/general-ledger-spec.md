...
| 銀行口座追跡 | `bank-account.yaml` で `status: none \| active` を明示。未宣言は拒否 |
| 銀行明細 | `status: none` ならスキップ。`active` ではファイル必須・当月行1件以上・未消込0・統制勘定突合（bank-tieout） |
...
| 月次 YAML 突合 | 計画が無い場合は warning（通過可）。計画があるのに元帳と差がある場合は error |
| 直前月 | 決算月の最初の月を除き、直前月が evidence 付きロック済みで仕訳ハッシュ一致（初月除く） |
