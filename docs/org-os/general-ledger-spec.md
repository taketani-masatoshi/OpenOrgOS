...
| 消費税 | 全仕訳に課税区分があり、課税事業者判定根拠・本則課税の控除方式が確定していること。固定資産等の課税仕入も集計し、取引別のインボイス状態・用途区分不足を警告する |
...
会社法の計算書類（貸借対照表・損益計算書・株主資本等変動計算書・個別注記表）と剰余金の処分は、総勘定元帳と `data/finance/year-end.FY####.yaml` から組む。表示の正本は `src/lib/finance/ledger/statutory-statements.ts` である。貸借は残高 0 の区分も残す。損益の `extraordinary` は特別損失、`extraordinary_gain` は特別利益である。非公開・会計監査人非設置の個別注記は会社計算規則第98条2項1号で省略されない7区分だけを別関数で出し、年次締めが見る後発事象の欠落条件は変えない。第101条の評価基準・減価償却・引当金・収益費用の計上基準は台帳か年度宣言の `statutory_notes` であり、無い項目は計算書類だけを未完にする。利益準備金は純関数で計算し、仕訳は切らない。剰余金処分の宣言が無い計算書類は未完だが、年度決算ゲートには加えない。税額の XML は計算書類の入力にしない。予実 YAML が無くても `report kessan --basis gl` は元帳から組む。
...
`evaluateTaxAdjustment` は、番号の付いた別表四・別表五（一）・別表一の行と、見積ではない法人税額を返す。仕訳は切らない。開始残高の切替もしない。

起点は損益振替前の当期純利益である。振替仕訳があるときは、その利益剰余金行から復元し、振替後の 0 は使わない。自動加算は、資産に書いた税務償却額を超える帳簿償却（別表四 行6）と、`tax-profile` に書いた交際費科目が上限を超えた差額（別表四 行8）だけである。明示行は `form_row` が別表四の許可行のときだけ行11または行22に入る。行番号が無い明示行があるときは、行11・行22・行23・行52と別表一を出さない。行52は、未配置の明示行が無く、行23と一致するときだけ出す。別表五（一）は利益剰余金の資本取引が 0 のときだけ、繰越損益金の行25と差引合計の行31を出す。法人税額はプロファイルの見積ではなく、行52の所得を千円未満切捨てした課税標準に、事業年度の月数で按分した800万円以下へ15%、超へ23.2%を適用し、百円未満を切り捨てる。適用除外のときは税額を出さない。課税所得が計算できないときは、別表に加算 0 と当期純利益の写しを出さない。手渡し XML は提出用ではない。
