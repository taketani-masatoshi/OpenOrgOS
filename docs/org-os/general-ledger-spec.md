...
`evaluateTaxAdjustment` は別表四・別表五（一）・別表一の公式行と、見積ではない法人税額を返す。仕訳は切らない。開始残高の切替もしない。

起点は損益振替前の当期純利益である。振替仕訳があるときは、その利益剰余金行から復元し、振替後の 0 は使わない。自動加算は、資産に書いた税務償却額を超える帳簿償却（別表四 行6）と、`tax-profile` に書いた交際費科目が上限を超えた差額（別表四 行8）だけである。それ以外は `data/finance/tax-adjustments.yaml` の明示行で、公式行番号は付けない。所得金額は別表四 行52。法人税額はプロファイルの見積ではなく、その所得を千円未満切捨てした課税標準に、別表一次葉の15%（年800万円以下）と23.2%を適用し、百円未満を切り捨てる。課税所得が計算できないときは、別表に加算 0 と当期純利益の写しを出さない。手渡し XML は提出用ではない。
