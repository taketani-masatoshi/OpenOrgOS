...
個人事業（`entity_form: sole_proprietorship`）の青色申告決算書（一般用）と所得税は別表四を使わない。行の正本は `jp_tax_individual` の `blue-return-line-map`。出力は `submission: not-for-etax` の顧問ドラフトで、送信はしない。
