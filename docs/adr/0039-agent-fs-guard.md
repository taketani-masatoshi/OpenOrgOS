...
6. Non-production: until `orgos guard init`, the gate is off (backward compatible). `ORGOS_FS_GUARD=off` disables; `enforce` requires init. **Production** refuses startup, `orgos doctor`, and **canonical write boundaries** (`wrapCanonicalWrite`, `applyAgentWrite`) if uninitialized, and forbids `ORGOS_FS_GUARD=off`. `requireCliOperator` also fails early. `guard init` is the exception. Platform paths (`data/.orgos/`, `data/chat/`, `data/scratch/`) skip prod/init checks so init can bootstrap. **`runFsGuardInternal`** (not exported from the public module) skips the write hook for guard-runtime ledger writes (`saveIdentities`, grant events, apply audit) so `guard apply` / `guard init` do not fail after writing the target file. Bootstrap paths `data/org/operators.yaml` and `data/org/access-grants.yaml` skip prod-ready checks before init while remaining **agent_forbidden** for Agent context.
7. Shell dispatch cwd is the AIA run workspace (`data/scratch/aia-runs/{run_id}`). `runtime.yaml` keeps `{tenant_root}` for profile integrity; **`runShellDispatch` overwrites cwd** when enforced. Redirects/`cp`/`mv`/`install` onto canonical paths are rejected when **argv[0] is a shell interpreter** (`bash`, `sh`, `zsh`, `dash`, `ksh`); non-shell argv (e.g. `echo` help text mentioning `docs/…`) is not scanned. **argv[0] allowlist** (echo · aider · cat from `runtime.yaml`) blocks interpreter one-liners (`python -c`, `node -e`). Child processes (aider) can still write arbitrary absolute paths — residual risk; OS sandbox is a follow-up.
...
9. Unhooked direct `writeFileSync` / `cpSync` etc. are frozen via `canonical-write-baseline.ts` + `npm run check:canonical-writes` (baseline keys are `file:symbol` counts, not line numbers). Batch-1 writers (operators, access-grants, regulation-bindings, aia-queue, routing-queue results, scheduling handoffs) now use hooked writers.
