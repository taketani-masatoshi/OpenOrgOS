...
4. **Assurance by tier** — When jurisdiction tier is **B or C** (amount-based), `approveOrgApproval` / Chat approve / Wire approve require a verified settlement WebAuthn assertion **in addition to** HumanApprovalContext (ADR 0038). Tier **A** and amount-less subjects still require HumanApprovalContext from a human issuer.
