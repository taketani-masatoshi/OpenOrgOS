...
4. **Assurance by tier** — When jurisdiction tier is **B or C** (amount-based), `approveOrgApproval` / Chat approve / Wire approve require a verified settlement WebAuthn assertion. Tier **A** and amount-less subjects (e.g. `tenant.config`) remain session + RBAC.
