...
5. **会計は `framework: financial`。** 実在・網羅・評価・期間帰属・表示を GL / period-lock / 補助元帳 / 月次締めへ載せる。表示健全性（presentation-sanity）は機械ルールのみで LLM は判定せず、baseline 更新は CLI `--update-baseline` のみ（workpapers は非更新）。外部会計監査人の代替ではない。**国税庁 HTML のスクレイプ結果を税率・控除へ自動適用しない** — `rules-freshness.yaml` の人手レビューと `sole-prop-blue rules-watch`（本文ハッシュ変更検知）のみ。
