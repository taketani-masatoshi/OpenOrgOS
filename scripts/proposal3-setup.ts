...
console.log(
  `  sudo cp ${join(ROOT_DIR, "deploy/proposal3/systemd/steward-org-c-api.service")} /etc/systemd/system/`
);
console.log(
  `  sudo cp ${join(ROOT_DIR, "deploy/proposal3/env/org-c-api.generated.env")} /etc/steward/org-c-api.env`
);
console.log("  sudo systemctl enable --now steward-org-c-api");
console.log("\n--- launchd（macOS · Mac mini 当事者） ---");
console.log(
  `  cp ${join(ROOT_DIR, "deploy/proposal3/launchd/com.steward.party-relay.plist")} ~/Library/LaunchAgents/`
);
