#!/usr/bin/env zsh
# Monthly OrgOS pipeline (records_audit monthly audit). Requires events:write operator.
set -euo pipefail
export PATH="/opt/homebrew/bin:/usr/local/bin:/usr/bin:/bin:${PATH:-}"

ROOT="$(cd "$(dirname "$0")/.." && pwd)"
TENANT="${ORGOS_TENANT:-mal}"
LOG="/tmp/orgos-pipeline-monthly-${TENANT}.log"
...
{
  echo "=== $(date -Iseconds) pipeline monthly tenant=${TENANT} ==="
