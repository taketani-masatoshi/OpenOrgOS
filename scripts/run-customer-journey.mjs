...
try {
  ...
  diffSha256 = createHash("sha256").update(execFileSync("git", ["diff", "HEAD", "--", ".", ":(exclude)tenants"], { cwd: root })).digest("hex");
...
try {
  const result = spawnSync(process.execPath, [
    ...
    "--config", "vitest.customer-journey.config.ts",
    "--reporter=default", "--reporter=json",
    `--outputFile.json=${join(runDir, "vitest.json")}`,
  ], { cwd: root, env, stdio: "inherit", timeout: 180_000, killSignal: "SIGKILL" });
  let report;
  try { report = JSON.parse(readFileSync(join(runDir, "vitest.json"), "utf8")); } catch { /* Failed before reporting. */ }
  // Empty, skipped or interrupted runs cannot become positive evidence.
  const passed = result.status === 0 && report?.success === true &&
  ...
  writeFileSync(join(runDir, "evidence.json"), JSON.stringify({
    ...
    source: { head, tracked_diff_sha256: diffSha256,
      harness_sha256: createHash("sha256").update([
        "scripts/run-customer-journey.mjs", "vitest.customer-journey.config.ts",
        "tests/customer-journey-http.test.ts",
      ].map((path) => readFileSync(join(root, path))).join("\n")).digest("hex") },
    passed,
    exit_code: result.status,
    signal: result.signal,
    tests: report ? { total: report.numTotalTests, passed: report.numPassedTests, failed: report.numFailedTests } : null,
    duration_ms: Date.now() - Date.parse(startedAt),
    limitations: ["synthetic session; no Passkey ceremony", "CSRF and settlement step-up disabled", "no browser, external AI, payment or customer outcome verification"],
  }, null, 2) + "\n");
