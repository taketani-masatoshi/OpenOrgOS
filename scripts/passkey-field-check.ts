...
function parseArgs(): { url: string; json: boolean; scope: "chat" | "wire" | "all" } {
  ...
  if (!url) {
    console.error(
      "Usage: passkey:field-check -- --url https://operator.example.com [--json] [--scope chat|wire|all]",
    );
    process.exit(1);
  }

  return { url: url.replace(/\/$/, ""), json, scope };
}
