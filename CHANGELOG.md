...
- ログイン画面が ID/パスワードだけに戻っていた経路をやめ、Community・PassKey・ID/パスワードを同じ画面に出す。
- コンソール右上の設定アイコンを太陽型から歯車に差し替えた。
- 「読み込み中…」を全画面で長く出さない。ログインは 150ms でフォームを出し、セッション確認は 4 秒で打ち切る。ページ側は見出しの下に短いインジケータだけ出す。
