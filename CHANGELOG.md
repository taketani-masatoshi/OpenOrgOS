...
- **個人事業主 year-end 処理** — 暦月仕訳カバレッジ・period-lock 網羅・消費税期末振替（`JE-CT-YE-{year}` → 2180 未払）・源泉 YAML↔GL 突合を warning で検知。`sole-prop-blue year-end-status` · `tax-consumption year-end-reclass` · `withholding reconcile`。空月は `journal_coverage.acknowledge_empty_months` で抑制。KLab 固有仕訳は追加しない。
