...
**版:** 2026-06-08 · **対象:** Steward OS フレームワーク（テナント非依存）

会社固有の法人名・事業・規程は **テナント**（`tenants/{id}/`）に置く。Agent はアクティブテナントの [rules/company_context.md](../../tenants/) を参照する。
...
エージェントは **提案と下書き** のみ。最終決定は **人間（オーナー）** が行う。

---

## 関連

- [openorgos-engineering-constitution.md](openorgos-engineering-constitution.md) — 長期保守のエンジニアリング憲章（SSOT · Event First 等）
