...
describe("dependency-graph", () => {
  ...
  it("resolves CTR-008 contract file to nodes", () => {
    const graph = loadDependencyGraph();
    const nodes = resolveSourceNodes(graph, "cursor/data/contracts/CTR-008.yaml");
    expect(nodes.some((n) => n.id === "cursor/data/contracts/CTR-008.yaml")).toBe(true);
  });

  it("impact from CTR-008 includes loans.yaml and contract CSV", () => {
    const graph = loadDependencyGraph();
    const { sources, impacts } = computeImpact(graph, "cursor/data/contracts/CTR-008.yaml");

    expect(sources.length).toBeGreaterThan(0);
    const targetIds = impacts.map((i) => i.nodeId);
    expect(targetIds).toContain("cursor/data/finances/loans.yaml");
    expect(targetIds).toContain("docs/data/契約管理表.csv");
  });
  ...
  it("impact from PROP-002 operating_costs reaches yojitsu expense", () => {
    const graph = loadDependencyGraph();
    const { impacts } = computeImpact(graph, "cursor/data/properties/PROP-002.yaml");
    const targetIds = impacts.map((i) => i.nodeId);
    expect(targetIds).toContain("cursor/data/finances/loans.yaml");
    expect(
      targetIds.some(
  ...
  it("impact from fixed-costs reaches dashboard", () => {
    const graph = loadDependencyGraph();
    const { impacts } = computeImpact(graph, "cursor/data/finances/fixed-costs.yaml");
    const targetIds = impacts.map((i) => i.nodeId);
    expect(targetIds).toContain("report-dashboard");
    expect(targetIds).toContain("cursor/data/plans/yojitsu-fy2026.yaml");
  });
