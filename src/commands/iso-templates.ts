...
} from "../lib/compliance/iso/templates.js";
...
function printPlan(plan: IsoTemplatePlan, wrote: boolean): void {
  ...
  for (const row of plan.rows) {
    console.log(
      `| ${row.file} | ${row.action === "create" ? (wrote ? "配置" : "未配置") : "既存（保持）"} |`
    );
  }
...
export function runIsoTemplates(standard: string, options: IsoTemplatesCliOptions = {}): void {
