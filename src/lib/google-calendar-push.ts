...
export interface GoogleCalendarPushResult {
  ...
  skipped: number;
  dryRun: boolean;
  events: Array<{ id: string; action: "create" | "update" | "skip"; googleEventId?: string }>;
}
...
export async function pushEventToGoogleCalendar(
...
): Promise<{ action: "create" | "update" | "skip"; googleEventId?: string }> {
  ...
  if (googleEventId) {
    ...
    return { action: "update", googleEventId: data.id };
  }
  ...
  return { action: "create", googleEventId: data.id };
}
  ...
  for (let i = 0; i < updatedEvents.length; i++) {
    ...
    if (push.action === "skip") {
      ...
      if (push.googleEventId) {
        updatedEvents[i] = { ...event, google_event_id: push.googleEventId };
      }
    } else {
      updated++;
    }
    details.push({ id: event.id, action: push.action, googleEventId: push.googleEventId });
  }
