...
import { listIsoStandardIds } from "../packs/paths.js";
import { listIsoCatalogEntries } from "./catalog.js";
...
export function inspectIsoMap(entry: IsoCatalogEntry): IsoMapStatus {
  ...
  if (entry.status === "coming_soon") return { ...base, skipped: true };
  if (!folder_ok) return { ...base, error: "pack folder missing" };
  ...
  try {
    ...
    const mismatch = controls.some(
      (control) => !control.iso_refs.some((ref) => ref.standard === entry.id)
    );
...
export function verifyIsoMaps(): { ok: boolean; statuses: IsoMapStatus[] } {
  ...
  return {
    ok: statuses.every((status) => status.skipped || (status.folder_ok && status.map_ok)),
    statuses,
  };
}
