...
export function buildAccountingReadinessChecks(
  ...
  return [
    ...
    {
      id: "month-close-ui-contract",
      gate: "A3",
      label: "Month-close API/UI contract present",
      weight: 3,
      pass:
        sourceIncludes("src/lib/steward-chat/routes/ledger-api.ts", ["month-close-checklist"]) &&
        sourceIncludes("apps/steward-chat/src/LedgerWorkbenchPage.tsx", ["monthCloseChecklist"]),
    },
    {
      id: "tax-handoff-contract",
      gate: "A2",
      label: "Advisor handoff contract present",
      weight: 3,
      pass:
        sourceIncludes("src/lib/tax/tax-handoff-package.ts", ["not-for-etax"]) &&
    ...
    },
    {
      ...
      detail: "Bookkeeping close and advisor handoff only",
    },
  ];
}
