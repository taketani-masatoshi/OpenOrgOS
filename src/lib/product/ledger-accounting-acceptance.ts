...
export function runIsolatedAccountingAcceptance(): AccountingAcceptanceResult {
  ...
  try {
    ...
    const bankEntries = months.map((month, index) => {
      const periodEnd = lastDayOfMonth(month);
      const closingBalance = buildTrialBalance({ asOf: periodEnd }).rows.find((row) => row.account_code === codes.bank_control)?.balance_yen ?? 0;
      const movement = closingBalance - openingBalance;
      const movements = movement === 0
        ? [{ suffix: "IN", direction: "inflow" as const, amount: 1 }, { suffix: "OUT", direction: "outflow" as const, amount: 1 }]
        : [{ suffix: "NET", direction: movement > 0 ? "inflow" as const : "outflow" as const, amount: Math.abs(movement) }];
      const entries = movements.map((row) => {
        const id = `BS-ACCEPTANCE-${String(index + 1).padStart(2, "0")}-${row.suffix}`;
        return {
          id, fingerprint: createHash("sha256").update(`${id}:${periodEnd}:${row.direction}:${row.amount}`).digest("hex"),
          date: periodEnd, direction: row.direction, amount: row.amount, category: "acceptance",
          description: `Certified acceptance movement ${month} ${row.suffix}`,
          account_id: "BANK-001", chart_account_id: codes.bank_control, import_batch_id: `BATCH-ACCEPTANCE-${month}`,
          status: "matched" as const, source: "import" as const,
        };
      });
      const batch = {
        id: `BATCH-ACCEPTANCE-${month}`,
        fingerprint: createHash("sha256").update(JSON.stringify(entries)).digest("hex"),
        imported_at: `${periodEnd}T23:59:59.000Z`, adapter: "accounting-acceptance",
        account_id: "BANK-001", period_start: `${month}-01`, period_end: periodEnd,
        opening_balance: openingBalance, closing_balance: closingBalance, entry_ids: entries.map((row) => row.id),
      };
      openingBalance = closingBalance;
      return { entries, batch };
    });
    writeYamlFileAtomic(join(getDataDir(), "finance", "bank-statements.yaml"), bankStatementFileSchema.parse({
      as_of: lastDayOfMonth(months.at(-1)!), currency: "JPY",
      entries: bankEntries.flatMap((row) => row.entries), import_batches: bankEntries.map((row) => row.batch),
      notes: "Generated by isolated accounting acceptance using the production bank-statement schema",
    }));
