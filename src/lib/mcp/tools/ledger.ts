...
export async function handleLedgerToday(_args: Record<string, unknown>): Promise<McpToolResult> {
  const { buildMonthCloseChecklist } =
    await import("../../product/ledger-month-close-checklist.js");
  const { listBankReconciliationWorkbench } = await import("../../finance/bank-reconcile-apply.js");
...
export async function handleLedgerProposeManualEntry(
  ...
  const { enqueueManualJournalProposal } = await import("../../product/ledger-proposal-queue.js");
  const proposal = enqueueManualJournalProposal({
    ...
    occurredAt: typeof args.occurred_at === "string" ? args.occurred_at : undefined,
    source: "mcp",
