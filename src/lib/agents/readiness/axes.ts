...
export type AgentPulseChecksResult = {
  ok: number;
  total: number;
  stale: string[];
};
...
export function scoreDefinition(agentId: AgentId): AgentReadinessAxis {
...
export function scoreOrchestration(agentId: AgentId): AgentReadinessAxis | null {
...
export function scoreEvidenceActivationBoundary(agentId: AgentId): AgentReadinessAxis {
  const agent = getCatalogAgent(agentId);
...
export function scoreTenant(cap: ReturnType<typeof getAgentCapability>): AgentReadinessAxis {
  const paths = [...(cap?.data_paths ?? []), ...(cap?.docs_paths ?? [])];
  if (paths.length === 0) {
...
export function scoreAdvisorDefinition(agentId: AgentId): AgentReadinessAxis {
  const agent = getCatalogAgent(agentId);
  const checks = [
