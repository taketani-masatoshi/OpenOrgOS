import { validateTrustedHubsRegistry } from "../distribution/trusted-hubs.js";
import {
  checkRevocationSla,
  loadTrustedOperatorsRegistry,
  validateTrustedOperatorsRegistry,
} from "../distribution/trusted-operators.js";
import {
...
export function computeCommunityReadiness(): CommunityReadiness {
  ...
  checks.push({
    ...
    ok: existsSync(join(getInstallRoot(), "src/lib/protocol/transport/peer-protocol-policy.ts")),
    detail: "contract protocol: peer whitelist",
  });
  if (existsSync(join(getInstallRoot(), "src/lib/protocol/transport/peer-protocol-policy.ts"))) score += 2;

  checks.push({
    id: "orgos-readiness-engine",
    ok: existsSync(join(getInstallRoot(), "src/lib/protocol/readiness/orgos-readiness.ts")),
    detail: "dynamic OrgOS scoring",
  });
  if (existsSync(join(getInstallRoot(), "src/lib/protocol/readiness/orgos-readiness.ts"))) score += 2;

  checks.push({
    id: "openorgos-core-readiness",
    ok: existsSync(join(getInstallRoot(), "src/lib/protocol/readiness/openorgos-core-readiness.ts")),
    detail: "Core four-element scoring",
  });
  if (existsSync(join(getInstallRoot(), "src/lib/protocol/readiness/openorgos-core-readiness.ts"))) score += 2;
  ...
  checks.push({
    ...
    ok: existsSync(join(getInstallRoot(), "src/lib/protocol/adapters/community-export.ts")),
    detail: "community read bundle export",
  });
  if (existsSync(join(getInstallRoot(), "src/lib/protocol/adapters/community-export.ts"))) score += 1;
