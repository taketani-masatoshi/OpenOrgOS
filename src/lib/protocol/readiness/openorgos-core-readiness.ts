...
export function computeOpenOrgOsCoreReadiness(): OpenOrgOsCoreReadiness {
  const eventModel = bucketScore([
    ...
    fileOk("src/lib/protocol/core/envelope.ts", "envelope serialize/parse"),
    fileOk("tests/protocol-org-event.test.ts", "org event tests"),
    fileOk("src/lib/protocol/core/record-transaction.ts", "transaction → envelope"),
  ...
  const identity = bucketScore([
    ...
    fileOk("src/lib/protocol/core/identity.ts", "identity export"),
    fileOk("src/lib/protocol/core/inbound-verify.ts", "inbound signature verify"),
  ...
  const auditability = bucketScore([
    fileOk("src/lib/protocol/core/audit-chain.ts", "hash-linked audit chain"),
    fileOk("src/lib/protocol/core/external-verify.ts", "external verify"),
