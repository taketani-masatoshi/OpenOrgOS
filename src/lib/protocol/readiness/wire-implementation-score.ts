...
export function evaluateWireImplementationChecklist(): WireImplementationScoreResult {
  const items: WireImplementationScoreItem[] = [
    ...
    scoreItem("trust-id", "pk-DID validate gates + slug reject", 12, [
      {
        ok: hasPattern("src/lib/protocol/core/validate.ts", /peer-slug-did-disallowed/),
        weight: 4,
        detail: "peer slug-DID gate missing in validate",
      },
      {
        ok: hasPattern("src/lib/protocol/distribution/wire-trust-registry.ts", /slug-did-disallowed/),
        weight: 4,
    ...
    scoreItem("openorg-dns", "OpenOrg DNS in transport/discover", 10, [
      {
        ok: hasPattern("src/lib/protocol/transport/transport.ts", /resolveOpenOrgWireUrl|resolvePeerInboundEndpointsWithDns/),
        weight: 5,
    ...
    scoreItem("multipath", "Multipath deliver tests pass", 12, [
      {
        ok: hasPattern("src/lib/protocol/transport/transport.ts", /email_wire|wire_v1|relay/),
        weight: 4,
    ...
    scoreItem("store-forward", "Flush backoff + dead-letter audit", 10, [
      {
        ok: hasPattern("src/lib/protocol/transport/transport.ts", /isWirePendingReadyForRetry/),
        weight: 3,
        detail: "flush does not respect next_retry_at",
      },
      {
        ok: hasPattern("src/lib/protocol/transport/transport.ts", /appendWireDeadLetterAudit/),
        weight: 3,
    ...
    scoreItem("r5-email", "Phase2 protocol CLI + prod gate", 12, [
      ...
      {
        ok: hasPattern("src/lib/protocol/readiness/prod-wire-gate.ts", /email_wire|evaluateEmailWireReadiness/),
        weight: 4,
    ...
    scoreItem("org-cert", "SPKI hash witness/PKIX integration", 10, [
      {
        ok: hasPattern("src/lib/protocol/distribution/org-cert-witness.ts", /loadWitnessTrustBundle|witness/),
        weight: 5,
    ...
    scoreItem("test-depth", "Static test-artifact checklist (not execution evidence)", 12, [
      {
        ok: readSource("src/lib/protocol/readiness/wire-implementation-score.ts").includes("scoreItem("),
        weight: 4,
        detail: "score lacks weighted category checks",
      },
      {
        ok: hasPattern("src/lib/protocol/readiness/wire-implementation-score.ts", /hasPattern|hasTestPattern/),
        weight: 4,
