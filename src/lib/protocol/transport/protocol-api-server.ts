...
async function handleProtocolApiRequest(
  ...
  const communityRoute = await (async () => {
    ...
    const { handleCommunityWireNodeApiRoute } = await import("../adapters/community-wire-node-api.js");
    ...
    if (wire) return wire;
    const { handleCommunityTenantMailApiRoute } = await import("../adapters/community-tenant-mail-api.js");
    ...
    if (mail) return mail;
    const { handleCommunityConnectorsApiRoute } = await import("../adapters/community-connectors-api.js");
