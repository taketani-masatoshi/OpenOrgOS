...
/** Optional trust-registry lookup — inject from distribution to avoid transport→distribution import. */
export type TrustWireUrlLookup = (
  nodeId: string
) => { wire_url?: string } | null | undefined;
...
export async function resolveOpenOrgWireUrl(
  ...
  const trust = opts?.trustLookup?.(trimmed);
  if (trust?.wire_url) {
    return result(trimmed, "trust-registry", trust.wire_url.replace(/\/$/, ""));
  }
