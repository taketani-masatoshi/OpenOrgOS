...
import { loadRelayState, saveRelayState } from "../transport/relay-state.js";
import {
  flushWirePending,
...
export { loadRelayState, saveRelayState } from "../transport/relay-state.js";
...
export async function runRelayCycle(opts?: {
  ...
  deliver?: DeliverProtocolEnvelopeOptions;
}): Promise<RelayCycleResult> {
  const wireFlushed = await flushWirePending(opts?.deliver);
...
export async function runRelayDaemon(opts: RelayDaemonOptions = {}): Promise<void> {
  ...
  while (cycles < maxCycles) {
    const result = await runRelayCycle({ reconcile: opts.reconcile, deliver: opts.deliver });
    console.log(
