...
import {
  ...
  businessUnitsFileSchema,
  type BusinessUnitsFile,
  payrollSchema,
  cashBalanceSchema,
  fixedAssetsSchema,
  taxProfileSchema,
  ...
  debtPlanSchema,
  bankFinancingCasesFileSchema,
  bankFinancingPitchFileSchema,
  capitalRaiseCasesFileSchema,
  fundingStrategyFileSchema,
...
export function loadFixedCosts(): FixedCosts {
  return readYamlFile(
    join(getDataDir(), "finance", "fixed-costs.yaml"),
    fixedCostsSchema,
  );
}
...
export function resolveCashBalanceTotal(balance: CashBalance): number | null {
  ...
  const amounts = balance.accounts
    .map((a) => a.amount)
    .filter((a): a is number => a != null);
  if (amounts.length === 0 || amounts.length < balance.accounts.length)
    return null;
...
export function loadFixedAssets(): FixedAssets {
  return readYamlFile(
    join(getDataDir(), "finance", "fixed-assets.yaml"),
    fixedAssetsSchema,
  );
}
...
export function loadTaxFilingGaps(): TaxFilingGaps {
  return readYamlFile(
    join(getDataDir(), "finance", "tax-filing-gaps.yaml"),
    taxFilingGapsSchema,
  );
}
...
export function validateFixedAssetConsistency(): FixedAssetConsistencyIssue[] {
  ...
  for (const asset of fixedAssets.assets) {
    if (!propertyById.has(asset.property_id)) {
      issues.push({
        message: `${asset.id}: property_id ${asset.property_id} not found`,
      });
    }
    if (asset.loan_id && !loanById.has(asset.loan_id)) {
      issues.push({
        message: `${asset.id}: loan_id ${asset.loan_id} not found`,
      });
    }
    const expectedBook =
  ...
  for (const loan of loans.loans) {
    if (loan.fixed_asset_ids) {
      for (const aid of loan.fixed_asset_ids) {
        if (!assetIds.has(aid)) {
          issues.push({
            message: `${loan.id}: fixed_asset_id ${aid} not in fixed-assets.yaml`,
          });
        }
      }
    }
  ...
  const fyId = fixedAssets.fiscal_year ?? "FY2026";
  const fyPlan = expensePlan.years.find((y) => y.fiscal_year === fyId);
  const expenseLinesById = new Map(
    (fyPlan?.lines ?? []).map((line) => [line.id, line]),
  );
  const depreciationLineIds = new Set(
    fixedAssets.assets
      .map((asset) => asset.expense_plan_line_id)
      .filter((id): id is string => id != null),
  );
  const expensePlanDepreciation = (fyPlan?.lines ?? [])
    .filter((line) => depreciationLineIds.has(line.id))
    .reduce((sum, line) => sum + line.amount, 0);
  if (
    depreciationLineIds.size > 0 &&
    fixedAssets.summary?.annual_depreciation_fy_current != null
  ) {
    if (
      expensePlanDepreciation !==
      fixedAssets.summary.annual_depreciation_fy_current
    ) {
      issues.push({
        message: `expense-plan ${fyId} depreciation (${expensePlanDepreciation}) ≠ fixed-assets annual_depreciation_fy_current (${fixedAssets.summary.annual_depreciation_fy_current})`,
      });
    }
  }

  let fyDepreciationSum = 0;
  let fyDepreciationCount = 0;
  for (const asset of fixedAssets.assets) {
    if (asset.category === "土地" || asset.depreciation_method === "非償却") {
      continue;
    }
    ...
    if (!lineId) {
      issues.push({
        message: `${asset.id}: fy_depreciation_jpy set but expense_plan_line_id missing`,
      });
      continue;
    }
    const line = expenseLinesById.get(lineId);
    if (!line) {
      issues.push({
        message: `${asset.id}: expense_plan_line_id ${lineId} not found in ${fyId}`,
      });
    } else if (line.amount !== asset.fy_depreciation_jpy) {
      issues.push({
        message: `${asset.id}: fy_depreciation_jpy (${asset.fy_depreciation_jpy}) ≠ expense-plan ${lineId} (${line.amount})`,
      });
    }
  }
  if (
    fyDepreciationCount > 0 &&
    fixedAssets.summary?.annual_depreciation_fy_current != null &&
    fyDepreciationSum !== fixedAssets.summary.annual_depreciation_fy_current
  ) {
    issues.push({
      message: `sum of fy_depreciation_jpy (${fyDepreciationSum}) ≠ summary.annual_depreciation_fy_current (${fixedAssets.summary.annual_depreciation_fy_current})`,
    });
  }

  if (fixedAssets.summary) {
    const calcCost = fixedAssets.assets.reduce(
      (s, a) => s + a.acquisition_cost,
      0,
    );
    const calcAccum = fixedAssets.assets.reduce(
...
export function loadBusinessPlan(): BusinessPlan {
  return readYamlFile(
    join(getDataDir(), "plans", "business-plan.yaml"),
    businessPlanSchema,
  );
}
...
export function loadRevenuePlan() {
  return readYamlFile(
    join(getDataDir(), "plans", "revenue-plan.yaml"),
    revenuePlanSchema,
  );
}

export function loadBusinessUnits(): BusinessUnitsFile | undefined {
  const path = join(getDataDir(), "plans", "business-units.yaml");
  try {
    return readYamlFile(path, businessUnitsFileSchema);
  } catch {
    return undefined;
  }
}
...
export function loadProfitPlan() {
  return readYamlFile(
    join(getDataDir(), "plans", "profit-plan.yaml"),
    profitPlanSchema,
  );
}
...
export function loadExpensePlan() {
  return readYamlFile(
    join(getDataDir(), "plans", "expense-plan.yaml"),
    expensePlanSchema,
  );
}
...
export function loadInvestmentPlan() {
  return readYamlFile(
    join(getDataDir(), "plans", "investment-plan.yaml"),
    investmentPlanSchema,
  );
}
...
export function loadDebtPlan() {
  return readYamlFile(
    join(getDataDir(), "plans", "debt-plan.yaml"),
    debtPlanSchema,
  );
}

export function loadBankFinancingCasesFile() {
  return readYamlFile(
    join(getDataDir(), "finance", "bank-financing-cases.yaml"),
    bankFinancingCasesFileSchema,
  );
}

export function loadBankFinancingPitchFile() {
  return readYamlFile(
    join(getDataDir(), "finance", "bank-financing-pitch.yaml"),
    bankFinancingPitchFileSchema,
  );
}

export function loadCapitalRaiseCasesFile() {
  return readYamlFile(
    join(getDataDir(), "finance", "capital-raise-cases.yaml"),
    capitalRaiseCasesFileSchema,
  );
}

export function loadFundingStrategyFile() {
  return readYamlFile(
    join(getDataDir(), "finance", "funding-strategy.yaml"),
    fundingStrategyFileSchema,
  );
}

export function loadPolicyFinanceCasesFile() {
  return readYamlFile(
    join(getDataDir(), "finance", "policy-finance-cases.yaml"),
    policyFinanceCasesFileSchema,
  );
}
...
export function loadEmployees(): EmployeesFile {
  return readYamlFile(
    join(getDataDir(), "hr", "employees.yaml"),
    employeesFileSchema,
  );
}
...
export function loadOneOnOnes(): OneOnOnesFile {
  return readYamlFile(
    join(getDataDir(), "executive", "one-on-ones.yaml"),
    oneOnOnesFileSchema,
  );
}
...
export function loadStakeholders(): StakeholdersFile {
  return readYamlFile(
    join(getDataDir(), "executive", "stakeholders.yaml"),
    stakeholdersFileSchema,
  );
}
...
export function validateAll(): { ok: boolean; errors: ValidationError[] } {
  ...
  if (existsSync(join(getDataDir(), "finance", "bank-financing-pitch.yaml"))) {
    tryLoad("data/finance/bank-financing-pitch.yaml", () =>
      loadBankFinancingPitchFile(),
    );
  }
  if (existsSync(join(getDataDir(), "finance", "capital-raise-cases.yaml"))) {
    tryLoad("data/finance/capital-raise-cases.yaml", () =>
      loadCapitalRaiseCasesFile(),
    );
  }
  if (existsSync(join(getDataDir(), "finance", "funding-strategy.yaml"))) {
    tryLoad("data/finance/funding-strategy.yaml", () =>
      loadFundingStrategyFile(),
    );
  }
  if (existsSync(join(getDataDir(), "finance", "policy-finance-cases.yaml"))) {
    tryLoad("data/finance/policy-finance-cases.yaml", () =>
      loadPolicyFinanceCasesFile(),
    );
  }
  ...
  for (const f of listYamlFiles(join(getDataDir(), "plans")).filter((p) =>
    p.includes("outlook-fy"),
  )) {
    tryLoad(toLogicalPath(f), () => readYamlFile(f, midYearOutlookSchema));
  }
  ...
  for (const mod of loadEnabledModules()) {
    if (mod.operations_public) {
      tryLoad(mod.operations_public, () =>
        readYamlFile(
          resolveTenantPath(mod.operations_public!),
          facilityPublicSchema,
        ),
      );
    }
  ...
  const executiveExt = join(
    getDataDir(),
    "executive",
    "external-contacts.yaml",
  );
  if (existsSync(executiveExt)) {
    tryLoad("data/executive/external-contacts.yaml", () =>
  ...
  const schedulingCases = join(
    getDataDir(),
    "executive",
    "scheduling-cases.yaml",
  );
  if (existsSync(schedulingCases)) {
    tryLoad("data/executive/scheduling-cases.yaml", () =>
      loadSchedulingCases(),
    );
  }
  ...
  if (existsSync(budgetDelegations)) {
    tryLoad("data/org/budget-delegations.yaml", () =>
      readYamlFile(budgetDelegations, budgetDelegationFileSchema),
    );
  }
  const laborDocuments = join(getDataDir(), "hr", "labor-documents.yaml");
  if (existsSync(laborDocuments)) {
    tryLoad("data/hr/labor-documents.yaml", () =>
      readYamlFile(laborDocuments, laborDocumentsFileSchema),
    );
  }
