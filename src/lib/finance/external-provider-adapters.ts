...
export function mapPayPalCaptureEvent(payload: any, tenant_id: string): ExternalFinanceTransaction[] {
  ...
  const minor_unit = iso4217MinorUnit(currency) ?? 2;
  return [{ tenant_id, transaction_id: String(resource.id), source: "paypal", type: "INCOME", amount: String(resource.amount.value), currency, payer_or_payee: String(resource.payer?.email_address ?? resource.seller_protection?.status ?? "UNKNOWN"), transaction_date: String(payload.create_time ?? new Date().toISOString()), direction: "CREDIT", minor_unit, raw_event_id: String(payload.id ?? resource.id), metadata: { event_type: String(payload.event_type) } }];
}
