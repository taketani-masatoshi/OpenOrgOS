...
**測定日:** 2026年8月30日 · **実測:** `orgos iso records check --tenant mal` · `orgos iso audit run --tenant mal` · `orgos iso requirements --tenant mal`

**最新の事前検査:** IAR-1788057497938-z6qlni3z（2026-08-30）· レポート [latest-iso-audit.md](../../audit/internal/latest-iso-audit.md) · 追記ログ `data/compliance/iso-internal-audit.jsonl`（25 行）
...
**ISO-13485 の位置づけ（2026-08-30 CEO 判断）:** 有効のまま、**認証も自己宣言も追わない**。
成熟度は L0 に据え、REG-025 / REG-026 と医療機器台帳（QMS · GVP）の運用だけを回す。
したがって事前検査に出る 13485 の不適合 10 件は「認証準備の未了」であり、
**薬機法側の規制対応の欠落ではない**。適用除外にしないのは、台帳と規程が現に
動いており、excluded にすると実態とずれるため。追う判断をした時点で、
成熟度の引き上げは運用実績を証拠パスに置いてから行う（自動のかさ上げはしない）。

### A層 記録の内容（73 記録）

| 状態 | 件数 | 意味 |
|------|:----:|------|
| 適合 | **10** | 様式が埋まり、記録仕様（`records.yaml`）を満たす |
| 不備 | **14** | ファイルはあるが内容が仕様違反 |
| 未作成 | **49** | 様式が配置されていない |
...
**9001（2026-08-30 に記入）:** 7 記録のうち **適合 4**（マネジメントレビュー議事 · 不適合な出力の記録 4 件 ·
リスク及び機会 8 件 · 品質目標）。残る 3 件は未作成（内部監査計画 · 是正処置記録 · プロセス相互作用図）で、
MR の決定事項に期限と担当を置いた。MR は初回であり、レビュー自体が「適合の主張ではなく記録の欠落の確認」で終わっている。
...
| P1 | 9001 の残り 3 記録（内部監査計画 · 是正処置記録 · プロセス相互作用図） | MR 決定事項の期限どおりに作成 |
